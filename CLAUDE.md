...
### ユーザーへの確認待ち事項
- Stripeのテスト用APIキー（`sk_test_...`）を共有してもらい、Checkout Session作成からE2Eで検証する
- 本番Vercelに `STRIPE_SECRET_KEY` / `STRIPE_WEBHOOK_SECRET` を設定してもらう（Webhookエンドポイント: `https://yoin-kappa.vercel.app/api/webhooks/stripe`、購読イベント: `checkout.session.completed`）
